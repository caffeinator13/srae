...
public class ExtractedEntitites {
	...
	/**	int ej = HashMap.get(jdexp.get(ej));
    	...
    	if(isInteger(jdexp1.toString())){
    		if(isInteger(cvexp1.toString())){
    			String jdexp = jdexp1.toString();
    			Integer x = Integer.valueOf(jdexp);
    			String cvexp = jdexp1.toString();
    			Integer y = Integer.valueOf(cvexp);
    			if(x>y){
    				System.out.println("you Don't have enough experience");
    			} else{
    				System.out.println("you have enough experience. checking the skill set...");
    			}
    		}
    	}
    ...
    public static void Compare(){
    	/** for(int i=0; i< jdskillsmap.size();i++){
    		for(int j = 0; j< cvskillsmap.size();j++){
    	...
    	for ( Entry<String, String> entry : jdskillsmap.entrySet() ) {
    		   if ( cvskillsmap.containsKey(entry.getKey()) ) {
    		      //if the key is common to both map1 and map2, compare the values
