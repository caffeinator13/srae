...
public class UpdateExisitingModel {
	// fill this method in with however you are going to get your data into a
	// list of sentences..for me I am hitting a MySQL database
	private static List<String> getSentencesFromSomewhere() throws Exception {
		...
		String sentence[] = Opennlp.SentenceDetect(handler.toString());
		// DocProvider dp = new DocProvider();
		/**
		 * String[] dp = Opennlp.SentenceDetect(handler.toString()); String
		 * modelPath = "c:\\apache\\entitylinker\\"; EntityLinkerProperties
		 * properties = new EntityLinkerProperties(new File(modelPath +
		 * "entitylinker.properties")); Map<Long, List<String>> docs =
		 * dp.getClass(properties); for (Long key : docs.keySet()) { counter++;
		 * System.out.println("\t\tDOC: " + key + "\n\n"); String docu = "";
		 * sentences.addAll(docs.get(key)); counter++; if(counter > 1000){
		 * break; } }
		 */
		return sentences;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		/**
		 * establish a file to put your NER hits in (the ones you want to keep
		 * based on prob)
		 */
		File knownEntities = new File("C:\\temp\\modelbuilder\\knownentities.txt");

		/**
		 * establish a BLACKLIST file to put your bad NER hits in (also can be
		 * based on prob)
		 */
		...
		// ------------create a bunch of file writers to write your results and
		// sentences to a file
		...
		// set some thresholds to decide where to write hits, you don't have to
		// use these at all...
		...
		TokenNameFinderModel personModel = new TokenNameFinderModel(
				new File("c:\\temp\\opennlpmodels\\en-ner-person.zip"));
		...
		for (String s : getSentencesFromSomewhere()) {
			...
			for (int i = 0; i < names.length; i++) {
				// YOU PROBABLY HAVE BETTER HEURISTICS THAN THIS TO MAKE SURE
				// YOU GET GOOD HITS OUT OF THE DEFAULT MODEL
		...
		/**
		 * THIS IS WHERE THE ADDON IS GOING TO USE THE FILES (AS IS) TO CREATE A
		 * NEW MODEL. YOU SHOULD NOT HAVE TO RUN THE FIRST PART AGAIN AFTER THIS
		 * RUNS, JUST NOW PLAY WITH THE KNOWN ENTITIES AND BLACKLIST FILES AND
		 * RUN THE METHOD BELOW AGAIN UNTIL YOU GET SOME DECENT RESULTS (A
		 * DECENT MODEL OUT OF IT).
		 */
		DefaultModelBuilderUtil.generateModel(sentences, knownEntities, blacklistedentities, theModel,
				annotatedSentences, "person", 3);
